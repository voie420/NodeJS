...
app.post("/upload", upload.single("videoFile"), function(req, res) {
  ...
  try {
    ...
    console.log("Resizing & uploading video from client.")
    ...
    .on("end", () => {
      ...
      // Send a response with the resized video filename
      res.status(200).send({ resizedFileName });
    })
    .on("error", (err) => {
  ...
  if (req.cookies.ACCESS_TOKEN) {
    ...
    connection.query('SELECT * FROM user_data', async (error, results, fields) => {
      ...
      await Promise.all(results.map(async (Result) => {
        const VALID = await bcrypt.compare(Result.USR_ACS_TOKEN, req.cookies.ACCESS_TOKEN);
        if (VALID) {
  ...
  if (sID != "") {
    ...
    connection.query('SELECT * FROM user_data WHERE USR_ID = ?', [userID], (error, results, fields) => {
      if (results.length == 1) {
        ...
        connection.end();
        bcrypt.compare(results[0].USR_ACS_TOKEN, req.cookies.ACCESS_TOKEN).then(VALID => {
          res.status(200).send({
            isOwner: VALID,
...
app.get('/followAcc', (req, res) => {
  ...
  if (id && sID) {
    ...
    connection.query('SELECT * FROM user_data WHERE USR_ID = ?', [sID], (error, results, fields) => {
      const ENC_ACCESS_TOKEN = req.cookies.ACCESS_TOKEN
    ...
    connection.query('SELECT * FROM user_data WHERE USR_ACS_TOKEN', [], (error, results, fields) => {
      ...
      if (results) {
        results.forEach(item => {
          bcrypt.compare(item.USR_ACS_TOKEN, req.cookies.ACCESS_TOKEN).then(VALID => {
            if (VALID) {
        ...
        if (SignedIn == false) {
          ...
          //ServerLog(`INVALID TOKEN: ${req.cookies.ACCESS_TOKEN}`)
          //res.clearCookie("ACCESS_TOKEN")
